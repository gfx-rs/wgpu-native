...
impl Drop for WGPUAdapterImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            ...
            context.global.adapter_drop(self.id);
        }
    }
}
...
impl Drop for WGPUBindGroupImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.bind_group_drop(self.id);
        }
    }
}
...
impl Drop for WGPUBindGroupLayoutImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.bind_group_layout_drop(self.id);
        }
    }
}
...
impl Drop for WGPUBufferImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.buffer_drop(self.id);
        }
    }
}
...
impl Drop for WGPUCommandBufferImpl {
    fn drop(&mut self) {
        if self.open.load(atomic::Ordering::SeqCst) && !thread::panicking() {
            let context = &self.context;
            context.global.command_buffer_drop(self.id);
        }
    }
}
...
impl Drop for WGPUCommandEncoderImpl {
    fn drop(&mut self) {
        if self.open.load(atomic::Ordering::SeqCst) && !thread::panicking() {
            let context = &self.context;
            context.global.command_encoder_drop(self.id);
        }
    }
}
...
impl Drop for WGPUComputePipelineImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.compute_pipeline_drop(self.id);
        }
    }
}
...
impl Drop for QueueId {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.queue_drop(self.id);
        }
    }
}
...
impl Drop for WGPUDeviceImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            ...
            context.global.device_drop(self.id);
        }
    }
}
...
impl Drop for WGPUPipelineLayoutImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.pipeline_layout_drop(self.id);
        }
    }
}
...
impl Drop for WGPUQuerySetImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.query_set_drop(self.id);
        }
    }
}
...
impl Drop for WGPURenderBundleImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.render_bundle_drop(self.id);
        }
    }
}
...
impl Drop for WGPURenderPipelineImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.render_pipeline_drop(self.id);
        }
    }
}
...
impl Drop for WGPUSamplerImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            context.global.sampler_drop(self.id);
        }
    }
}
...
impl Drop for WGPUShaderModuleImpl {
    fn drop(&mut self) {
        if let Some(id) = self.id {
            if !thread::panicking() {
                let context = &self.context;
                context.global.shader_module_drop(id);
            }
        }
    }
}
...
impl Drop for WGPUSurfaceImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            self.context.global.surface_drop(self.id);
        }
    }
}
    ...
    fn drop(&mut self) {
        ...
        if let Some(surface_id) = self.surface_id {
            if !self.has_surface_presented.load(atomic::Ordering::SeqCst) {
                match self.context.global.surface_texture_discard(surface_id) {
                    Ok(_) => (),
                    Err(cause) => handle_error_fatal(cause, "wgpuTextureRelease"),
                }
            }
        }
        self.context.global.texture_drop(self.id);
    }
}
...
impl Drop for WGPUTextureViewImpl {
    fn drop(&mut self) {
        if !thread::panicking() {
            let context = &self.context;
            let _ = context.global.texture_view_drop(self.id);
        }
    }
}
...
pub unsafe extern "C" fn wgpuAdapterGetFeatures(
    ...
    let adapter_features = context.global.adapter_features(adapter_id);
...
pub unsafe extern "C" fn wgpuAdapterGetLimits(
    ...
    let wgt_limits = context.global.adapter_limits(adapter_id);
...
pub unsafe extern "C" fn wgpuAdapterGetInfo(
    ...
    let result = context.global.adapter_get_info(adapter_id);
...
pub unsafe extern "C" fn wgpuAdapterRequestDevice(
    ...
    let adapter_limits = context.global.adapter_limits(adapter_id);
    ...
    let result = context
        .global
        .adapter_request_device(adapter_id, &desc, None, None);
    match result {
...
pub unsafe extern "C" fn wgpuBufferDestroy(buffer: native::WGPUBuffer) {
    ...
    context.global.buffer_destroy(buffer_id);
}
...
pub unsafe extern "C" fn wgpuBufferGetConstMappedRange(
    ...
    let buf = match context.global.buffer_get_mapped_range(
        buffer_id,
...
pub unsafe extern "C" fn wgpuBufferMapAsync(
    ...
    if let Err(cause) = context.global.buffer_map_async(
        buffer_id,
        offset as wgt::BufferAddress,
...
pub unsafe extern "C" fn wgpuBufferUnmap(buffer: native::WGPUBuffer) {
    ...
    if let Err(cause) = context.global.buffer_unmap(buffer_id) {
...
pub unsafe extern "C" fn wgpuCommandEncoderBeginComputePass(
    ...
    let (pass, err) = context
        .global
        .command_encoder_begin_compute_pass(command_encoder_id, &desc);
...
pub unsafe extern "C" fn wgpuCommandEncoderBeginRenderPass(
    ...
    let (pass, err) = context
        .global
        .command_encoder_begin_render_pass(command_encoder_id, &desc);
    if let Some(cause) = err {
        handle_error(
            error_sink,
            cause,
...
pub unsafe extern "C" fn wgpuCommandEncoderClearBuffer(
    ...
    if let Err(cause) = context.global.command_encoder_clear_buffer(
        command_encoder_id,
        buffer_id,
        offset,
        match size {
...
pub unsafe extern "C" fn wgpuCommandEncoderCopyBufferToBuffer(
    ...
    if let Err(cause) = context.global.command_encoder_copy_buffer_to_buffer(
        command_encoder_id,
        source_buffer_id,
        source_offset,
...
pub unsafe extern "C" fn wgpuCommandEncoderCopyBufferToTexture(
    ...
    if let Err(cause) = context.global.command_encoder_copy_buffer_to_texture(
        command_encoder_id,
...
pub unsafe extern "C" fn wgpuCommandEncoderCopyTextureToBuffer(
    ...
    if let Err(cause) = context.global.command_encoder_copy_texture_to_buffer(
        command_encoder_id,
...
pub unsafe extern "C" fn wgpuCommandEncoderCopyTextureToTexture(
    ...
    if let Err(cause) = context.global.command_encoder_copy_texture_to_texture(
        command_encoder_id,
        &conv::map_image_copy_texture(source.expect("invalid source")),
        &conv::map_image_copy_texture(destination.expect("invalid destination")),
        &conv::map_extent3d(copy_size.expect("invalid copy size")),
...
pub unsafe extern "C" fn wgpuCommandEncoderInsertDebugMarker(
    ...
    if let Err(cause) = context.global.command_encoder_insert_debug_marker(
        command_encoder_id,
...
pub unsafe extern "C" fn wgpuCommandEncoderPushDebugGroup(
    ...
    if let Err(cause) = context.global.command_encoder_push_debug_group(
        command_encoder_id,
...
pub unsafe extern "C" fn wgpuCommandEncoderResolveQuerySet(
    ...
    if let Err(cause) = context.global.command_encoder_resolve_query_set(
        command_encoder_id,
        query_set_id,
        first_query,
        query_count,
...
pub unsafe extern "C" fn wgpuCommandEncoderWriteTimestamp(
    ...
    if let Err(cause) = context.global.command_encoder_write_timestamp(
        command_encoder_id,
        query_set_id,
        query_index,
    ) {
...
pub unsafe extern "C" fn wgpuComputePassEncoderDispatchWorkgroups(
    ...
    match pass.context.global.compute_pass_dispatch_workgroups(
        encoder,
        workgroup_count_x,
        workgroup_count_y,
        workgroup_count_z,
...
pub unsafe extern "C" fn wgpuComputePassEncoderDispatchWorkgroupsIndirect(
    ...
    match pass
        .context
        .global
        .compute_pass_dispatch_workgroups_indirect(encoder, indirect_buffer_id, indirect_offset)
    {
        Ok(()) => (),
        Err(cause) => handle_error(
            &pass.error_sink,
...
pub unsafe extern "C" fn wgpuComputePassEncoderEnd(pass: native::WGPUComputePassEncoder) {
    ...
    match pass.context.global.compute_pass_end(encoder) {
        Ok(()) => (),
...
pub unsafe extern "C" fn wgpuComputePassEncoderInsertDebugMarker(
    ...
    match pass.context.global.compute_pass_insert_debug_marker(
        encoder,
        string_view_into_str(marker_label).unwrap_or(""),
        0,
    ) {
...
pub unsafe extern "C" fn wgpuComputePassEncoderPopDebugGroup(pass: native::WGPUComputePassEncoder) {
    ...
    match pass.context.global.compute_pass_pop_debug_group(encoder) {
        Ok(()) => (),
        Err(cause) => handle_error(
            &pass.error_sink,
            cause,
...
pub unsafe extern "C" fn wgpuComputePassEncoderPushDebugGroup(
    ...
    match pass.context.global.compute_pass_push_debug_group(
        encoder,
        string_view_into_str(group_label).unwrap_or(""),
        0,
    ) {
...
pub unsafe extern "C" fn wgpuComputePassEncoderSetBindGroup(
    ...
    match pass.context.global.compute_pass_set_bind_group(
        encoder,
        group_index,
        bind_group_id,
...
pub unsafe extern "C" fn wgpuComputePipelineGetBindGroupLayout(
    ...
    let (bind_group_layout_id, error) =
        context
            .global
            .compute_pipeline_get_bind_group_layout(pipeline_id, group_index, None);
    if let Some(cause) = error {
        handle_error(
            error_sink,
            cause,
...
pub unsafe extern "C" fn wgpuDeviceCreateBindGroup(
    ...
    let (bind_group_id, error) = context
        .global
...
pub unsafe extern "C" fn wgpuDeviceCreateBindGroupLayout(
    ...
    let (bind_group_layout_id, error) = context
        .global
...
pub unsafe extern "C" fn wgpuDeviceCreateBuffer(
    ...
    let (buffer_id, error) = context.global.device_create_buffer(device_id, &desc, None);
    if let Some(cause) = error {
...
pub unsafe extern "C" fn wgpuDeviceCreateComputePipeline(
    ...
    let (compute_pipeline_id, error) = context
        .global
        .device_create_compute_pipeline(device_id, &desc, None);
    if let Some(cause) = error {
...
pub unsafe extern "C" fn wgpuDeviceCreatePipelineLayout(
    ...
    let (pipeline_layout_id, error) = context
        .global
...
pub unsafe extern "C" fn wgpuDeviceCreateQuerySet(
    ...
    let (query_set_id, error) = context
        .global
        .device_create_query_set(device_id, &desc, None);
    if let Some(cause) = error {
...
pub unsafe extern "C" fn wgpuDeviceCreateRenderPipeline(
    ...
    let (render_pipeline_id, error) = context
        .global
...
pub unsafe extern "C" fn wgpuDeviceCreateSampler(
    ...
    let (sampler_id, error) = context.global.device_create_sampler(device_id, &desc, None);
    if let Some(cause) = error {
...
pub unsafe extern "C" fn wgpuDeviceCreateShaderModule(
    ...
    let (shader_module_id, error) = context
        .global
...
pub unsafe extern "C" fn wgpuDeviceCreateTexture(
    ...
    let (texture_id, error) = context.global.device_create_texture(device_id, &desc, None);
    if let Some(cause) = error {
...
pub unsafe extern "C" fn wgpuDeviceDestroy(device: native::WGPUDevice) {
    ...
    context.global.device_destroy(device_id);
}
...
pub unsafe extern "C" fn wgpuDeviceGetFeatures(
    ...
    let device_features = context.global.device_features(device_id);
    let features = features.expect("invalid return pointer \"features\"");
...
pub unsafe extern "C" fn wgpuDeviceGetLimits(
    ...
    let wgt_limits = context.global.device_limits(device_id);
    conv::write_limits_struct(wgt_limits, limits);
...
pub unsafe extern "C" fn wgpuInstanceCreateSurface(
    ...
    let surface_id = match create_surface_params {
        CreateSurfaceParams::Raw((rdh, rwh)) => {
            match context.global.instance_create_surface(rdh, rwh, None) {
        ...
        CreateSurfaceParams::Metal(layer) => {
            match context.global.instance_create_surface_metal(layer, None) {
                Ok(surface_id) => surface_id,
                Err(cause) => handle_error_fatal(cause, "wgpuInstanceCreateSurface"),
            }
        }
        ...
        CreateSurfaceParams::SwapChainPanel(panel) => {
            match context
                .global
                .instance_create_surface_from_swap_chain_panel(panel, None)
            {
                Ok(surface_id) => surface_id,
                Err(cause) => handle_error_fatal(cause, "wgpuInstanceCreateSurface"),
            }
...
pub unsafe extern "C" fn wgpuInstanceProcessEvents(instance: native::WGPUInstance) {
    ...
    match context.global.poll_all_devices(false) {
        Ok(_queue_empty) => (),
        Err(cause) => {
...
pub unsafe extern "C" fn wgpuInstanceRequestAdapter(
    ...
    match context.global.request_adapter(&desc, inputs, None) {
        Ok(adapter_id) => {
            callback(
...
pub unsafe extern "C" fn wgpuInstanceEnumerateAdapters(
    ...
    let result = context.global.enumerate_adapters(inputs);
    ...
    if !adapters.is_null() {
        ...
        result
            .iter()
            .for_each(|id| context.global.adapter_drop(*id));
    }
...
pub unsafe extern "C" fn wgpuQuerySetDestroy(query_set: native::WGPUQuerySet) {
    ...
    context.global.query_set_drop(query_set_id);
}
...
pub unsafe extern "C" fn wgpuQueueSubmit(
    ...
    if let Err(cause) = context.global.queue_submit(queue_id, &command_buffers) {
...
pub unsafe extern "C" fn wgpuQueueWriteBuffer(
    ...
    if let Err(cause) = context.global.queue_write_buffer(
        queue_id,
        buffer_id,
        buffer_offset,
...
pub unsafe extern "C" fn wgpuQueueWriteTexture(
    ...
    if let Err(cause) = context.global.queue_write_texture(
        queue_id,
        &conv::map_image_copy_texture(destination.expect("invalid destination")),
        make_slice(data.cast(), data_size),
...
pub unsafe extern "C" fn wgpuRenderBundleEncoderFinish(
    ...
    let (render_bundle_id, error) = context
        .global
        .render_bundle_encoder_finish(*encoder, &desc, None);
    if let Some(cause) = error {
...
pub unsafe extern "C" fn wgpuRenderPassEncoderDraw(
    ...
    match pass.context.global.render_pass_draw(
        encoder,
        vertex_count,
        instance_count,
        first_vertex,
...
pub unsafe extern "C" fn wgpuRenderPassEncoderDrawIndexed(
    ...
    match pass.context.global.render_pass_draw_indexed(
        encoder,
        index_count,
        instance_count,
        first_index,
...
pub unsafe extern "C" fn wgpuRenderPassEncoderDrawIndexedIndirect(
    ...
    match pass.context.global.render_pass_draw_indexed_indirect(
        encoder,
        indirect_buffer_id,
        indirect_offset,
    ) {
...
pub unsafe extern "C" fn wgpuRenderPassEncoderDrawIndirect(
    ...
    match pass.context.global.render_pass_draw_indirect(
        encoder,
        indirect_buffer_id,
        indirect_offset,
    ) {
...
pub unsafe extern "C" fn wgpuRenderPassEncoderEnd(pass: native::WGPURenderPassEncoder) {
    ...
    match pass.context.global.render_pass_end(encoder) {
        Ok(()) => (),
...
pub unsafe extern "C" fn wgpuRenderPassEncoderEndOcclusionQuery(
    ...
    match pass.context.global.render_pass_end_occlusion_query(encoder) {
        Ok(()) => (),
        Err(cause) => handle_error(
            &pass.error_sink,
            cause,
...
pub unsafe extern "C" fn wgpuRenderPassEncoderInsertDebugMarker(
    ...
    match pass.context.global.render_pass_insert_debug_marker(
        encoder,
        string_view_into_str(marker_label).unwrap_or(""),
        0,
    ) {
...
pub unsafe extern "C" fn wgpuRenderPassEncoderPopDebugGroup(pass: native::WGPURenderPassEncoder) {
    ...
    match pass.context.global.render_pass_pop_debug_group(encoder) {
        Ok(()) => (),
        Err(cause) => handle_error(
            &pass.error_sink,
            cause,
...
pub unsafe extern "C" fn wgpuRenderPassEncoderPushDebugGroup(
    ...
    match pass.context.global.render_pass_push_debug_group(
        encoder,
        string_view_into_str(group_label).unwrap_or(""),
        0,
    ) {
...
pub unsafe extern "C" fn wgpuRenderPassEncoderSetBindGroup(
    ...
    match pass.context.global.render_pass_set_bind_group(
        encoder,
        group_index,
        bind_group_id,
        make_slice(dynamic_offsets, dynamic_offset_count),
...
pub unsafe extern "C" fn wgpuRenderPassEncoderSetIndexBuffer(
    ...
    match pass.context.global.render_pass_set_index_buffer(
        encoder,
        buffer_id,
...
pub unsafe extern "C" fn wgpuRenderPassEncoderSetVertexBuffer(
    ...
    match pass.context.global.render_pass_set_vertex_buffer(
        encoder,
        slot,
        buffer_id,
        offset,
...
pub unsafe extern "C" fn wgpuRenderPipelineGetBindGroupLayout(
    ...
    let (bind_group_layout_id, error) =
        context
            .global
            .render_pipeline_get_bind_group_layout(render_pipeline_id, group_index, None);
    if let Some(cause) = error {
        handle_error(
            error_sink,
            cause,
...
pub unsafe extern "C" fn wgpuSurfaceConfigure(
    ...
    match context
        .global
        .surface_configure(surface.id, device.id, &surface_config)
    {
...
pub unsafe extern "C" fn wgpuSurfaceGetCurrentTexture(
    ...
    match context.global.surface_get_current_texture(surface.id, None) {
...
pub unsafe extern "C" fn wgpuSurfacePresent(surface: native::WGPUSurface) -> native::WGPUStatus {
    ...
    let _status = match context.global.surface_present(surface.id) {
        Ok(status) => status,
        Err(cause) => {
...
pub unsafe extern "C" fn wgpuTextureCreateView(
    ...
    let (texture_view_id, error) = context.global.texture_create_view(texture_id, &desc, None);
    if let Some(cause) = error {
...
pub unsafe extern "C" fn wgpuTextureDestroy(texture: native::WGPUTexture) {
    ...
    // Per spec, no error to report. Even calling destroy multiple times is valid.
    context.global.texture_destroy(texture_id);
}
...
pub unsafe extern "C" fn wgpuGenerateReport(
    ...
    conv::write_global_report(native_report, &context.global.generate_report());
}
...
pub unsafe extern "C" fn wgpuQueueSubmitForIndex(
    ...
    match context.global.queue_submit(queue_id, &command_buffers) {
...
pub unsafe extern "C" fn wgpuDevicePoll(
    ...
    match context.global.device_poll(device_id, maintain) {
...
pub unsafe extern "C" fn wgpuDeviceCreateShaderModuleSpirV(
    ...
    let (shader_module_id, error) = context
        .global
        .device_create_shader_module_passthrough(device_id, &desc, None);
    if let Some(cause) = error {
        handle_error(
            error_sink,
            cause,
...
pub unsafe extern "C" fn wgpuRenderPassEncoderSetPushConstants(
    ...
    match pass.context.global.render_pass_set_push_constants(
        encoder,
...
pub unsafe extern "C" fn wgpuComputePassEncoderSetPushConstants(
    ...
    match pass.context.global.compute_pass_set_push_constants(
        encoder,
        offset,
        make_slice(data, size_bytes as usize),
    ) {
...
pub unsafe extern "C" fn wgpuRenderPassEncoderMultiDrawIndirectCount(
    ...
    match pass.context.global.render_pass_multi_draw_indirect_count(
        encoder,
        buffer_id,
        offset,
        count_buffer_id,
...
pub unsafe extern "C" fn wgpuComputePassEncoderBeginPipelineStatisticsQuery(
    ...
    match pass
        .context
        .global
        .compute_pass_begin_pipeline_statistics_query(encoder, query_set_id, query_index)
    {
        Ok(()) => (),
        Err(cause) => handle_error(
            &pass.error_sink,
...
pub unsafe extern "C" fn wgpuRenderPassEncoderBeginPipelineStatisticsQuery(
    ...
    match pass
        .context
        .global
        .render_pass_begin_pipeline_statistics_query(encoder, query_set_id, query_index)
    {
        Ok(()) => (),
        Err(cause) => handle_error(
            &pass.error_sink,
